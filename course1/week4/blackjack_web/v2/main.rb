...
helpers do
  ...
  def player_wins!(message)
    @winner = "<strong>#{message}</strong> #{session[:player_name]} has #{calc_hand_total(session[:player_cards])}. Dealer has #{calc_hand_total(session[:dealer_cards])}"
    session[:player_chips] = session[:player_chips] + session[:player_bet_amount]
    @play_again = true
  end

  def player_loses!(message)
    @loser = "<strong>#{message}</strong> #{session[:player_name]} has #{calc_hand_total(session[:player_cards])}. Dealer has #{calc_hand_total(session[:dealer_cards])}"
    session[:player_chips] = session[:player_chips] - session[:player_bet_amount]
    @play_again = true
  end
...
post '/set_player_name' do
  ...
  session[:player_chips] = PLAYER_STARTING_CHIPS
  redirect '/make_bet'
end
...
post '/make_bet' do
  if params[:player_bet_amount].empty?
    @error = "Please bet an amount."
    halt erb(:make_bet)
  elsif params[:player_bet_amount].to_i > session[:player_chips]
    @error = "Please bet an amount that is no more than " + session[:player_chips].to_s
    halt erb(:make_bet)
  elsif params[:player_bet_amount].to_i == PLAYER_MIN_BET
    @error = "Please bet an amount more than " + PLAYER_MIN_BET.to_s
    halt erb(:make_bet)
  end
